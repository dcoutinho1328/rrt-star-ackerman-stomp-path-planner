...
class Vehicle:
    def __init__(
        self, width, length, axis_distance, max_steering_angle, collision_padding=1, back_traction=True
    ):
        self.width = width
        self.length = length
...
class Planner:
    def __init__(
        ...
        self.conservative_collision = False #self.check_problem()
    ...
    def create_padding_box(self, b, f, s, st, e):
        ...
        p1 = [st[0] - b*math.cos(t) + s*math.sin(t), st[1] - b*math.sin(t) - s*math.cos(t)]
        p2 = [st[0] - b*math.cos(t) - s*math.sin(t), st[1] - b*math.sin(t) + s*math.cos(t)]
        p3 = [e[0] + f*math.cos(t) - s*math.sin(t), e[1] + f*math.sin(t) + s*math.cos(t)]
        p4 = [e[0] + f*math.cos(t) + s*math.sin(t), e[1] + f*math.sin(t) - s*math.cos(t)]
    ...
    def check_collision(self, n1, n2=None):
        if self.conservative_collision:
            ...
            col = self.create_padding_box(back_padding, front_padding, side_padding, [st.x, st.y, st.yaw], [e.x, e.y, e.yaw])
    ...
    def check_collision_coords(self, p1, p2):
        if self.conservative_collision:
            ...
            col = self.create_padding_box(back_padding, front_padding, side_padding, p1, p2)
        ...
        while True:
            ...
            if (
                ...
                if self.check_collision(self.node_list[-1], self.goal):
                    ...
                    # elif self.max_retries > self.retries:
                    #     self.retries += 1
                    #     self.__clean()
                    #     continue
                    break
    ...
    def post_processing(self):
        ...
        for i in range (len(optimized) - 3):
            ...
            if np.sign(m[0] - s[0]) != np.sign(e[0] - m[0]):
                ...
                new_seg_start = [m[0] - dsegs*math.cos(m[2]), m[1] - dsegs*math.sin(m[2]), m[2], c]
                new_seg_end = [m[0] + dsegs*math.cos(e[2]), m[1] + dsegs*math.sin(e[2]), e[2]]
                if self.check_collision_coords(new_seg_start, new_seg_end):
                    optimized = optimized[:i + 1] + [new_seg_start, new_seg_end] + optimized[i+2:]
